...
common **R** way of fitting statistical models.

### t-test

We'll keep going with our Iris data; we want to test if petal lengths are 
significantly different between Iris setosa and Iris virginica; so we can use a 
...
Since we're only interested in setosa and virginica, we need to get rid of 
versicolor.
...
The basic linear regression model can be expressd as

$$ y_i = \alpha + \beta x_i + \epsilon_i $$

where $\alpha$ is the intercept and $\beta$ is the slope of the line and the 
error terms ($\epsilon_i$) are independent and normally distributed with equal 
variance.
...
A linear regression model is only valid if its residuals are consistent with the 
assumed error structure. We can check these assumptions by plotting the residuals 
...
diagnostic plots is created to help us check the assumptions of `lm()`.
