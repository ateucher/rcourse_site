...
You can get output from R simply by typing in math into the console
...
First, look in the files pane of RStudio, then see where we are in R.

```{r eval=FALSE}
getwd()
```

The working directory is the place where you're working, and where R looks for 
things in input/output operations.

## Scripts

* Record your code in scripts, rather than typing directly into the console
* Comment your code liberally using `#`.  Your future self will thank you.
* Break your code into chunks - by file, and within a file using lines of `#`

## Loading Data

To load our iris data, we need to download our data to a place we can use it. 
We will use `dir.create()` to create a `data` directory in our project directory, 
and `read.csv()` to load the file. These are built-in functions in R. Let's 
...
We can use the function `paste()` to create strings made up from different 
objects, if we separate them with commas.
...
vectors of length 1. A vector is the most common and basic data structure in `R` 
...
**Question:** Given what we learned about how to access a single column, and 
indexing vectors, what is the 19th value in the Sepal Length column? How did you 
find it?
