...
dat <- read.csv("data/iris.csv", stringsAsFactors = FALSE)

plot(dat$Sepal.Width, dat$Sepal.Length, type="p", col=factor(dat$Species))
```

You can also specify them in a formula format `plot(y ~ x, data='', ...)`
```{r baseplot2}
plot(Sepal.Length ~ Sepal.Width, data=dat, col=factor(Species))
...
```{r irisreshapeviolin, warning=FALSE}
library(reshape2)
...
In the fall 2014 course, we used the function `gather` in the package `tidyr`, 
which is similar to `reshape2` but does fewer things so is generally a bit simpler: 

```{r irisreshapebox, warning=FALSE}
library("tidyr")

# You can either tell it what columns to *not* gather by preceding it by a 
# minus sign (e.g., -Species):
long_dat <- gather(data = dat, key = measurement_type, 
                   value = measurement, -Species)

# Or specify which columns to gather by giving it a range of columns (e.g., 1:4):
long_dat <- gather(data = dat, key = measurement_type, 
                   value = measurement, 1:4)

ggplot(long_dat, aes(x = measurement_type, y = measurement)) + 
  geom_boxplot(aes(colour = measurement_type)) + 
  facet_wrap(~Species)

ggplot(long_dat, aes(x = Species, y = measurement)) + 
