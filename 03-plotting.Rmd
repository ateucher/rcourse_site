...
R's base (vuilt-in) plotting functions are powerful and very flexible, but not 
...
Now, plot a graphs using the iris dataset using **ggplot2**. You don't have to 
...
What if we want to split up the measurements and compare them all?  We need 
measurement as a variable... There is another package by Hadley Wickham (see a 
trend here?) that helps us restructure, or reshape our data: It's called 
`reshape2`.
